/*************************************************************************
 ...
 > Input: ./mix_noise_gaussian pepper.raw pepper_noisy.raw 512 512
 ************************************************************************/
...
//			printf("%u ", Imagetrans[i][j][0]);
	...
	for(int i=0; i<Width; i++)
		for(int j=0; j<Length; j++)
			for(int k=0; k<BytesPerPixel; k++)
				Imagedata4[i][j][k] = (Imagedata3[i][j][k]+2*Imagedata3[i][j+1][k]+Imagedata3[i][j+2][k]+2*Imagedata3[i+1][j][k]+4*Imagedata3[i+1][j+1][k]+2*Imagedata3[i+1][j+2][k]+Imagedata3[i+2][j][k]+2*Imagedata3[i+2][j+1][k]+Imagedata3[i+2][j+2][k])/16;
	...
	if (!(file=fopen("pepper_gaussian.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_gaussian.raw" << endl;
		exit(1);
	}
