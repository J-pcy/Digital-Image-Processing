/*************************************************************************
 ...
 > Input: ./mix_noise_median pepper.raw pepper_noisy.raw 512 512
 ************************************************************************/
...
unsigned char MEDIAN(unsigned char A, unsigned char B, unsigned char C, unsigned char D, unsigned char E, unsigned char F, unsigned char G, unsigned char H, unsigned char I);
...
//			printf("%u ", Imagetrans[i][j][0]);
	...
	for(int i=0; i<Width; i++)
		for(int j=0; j<Length; j++)
			for(int k=0; k<BytesPerPixel; k++)
				Imagedata4[i][j][k] = MEDIAN(Imagedata3[i][j][k], Imagedata3[i][j+1][k], Imagedata3[i][j+2][k], Imagedata3[i+1][j][k], Imagedata3[i+1][j+1][k], Imagedata3[i+1][j+2][k], Imagedata3[i+2][j][k], Imagedata3[i+2][j+1][k], Imagedata3[i+2][j+2][k]);
	...
	if (!(file=fopen("pepper_median.raw","wb"))) {
		cout << "Cannot open file: " << "pepper_median.raw" << endl;
		exit(1);
	}
