...
int main(int argc,char** argv)
{
 ...
 //image_src=imread("convertible_1.raw");
    if (!(image = fopen("convertible_1.raw", "rb"))) {
        cout << "Cannot open file: " << "convertible_1.raw" << endl;
        exit(1);
    }
...
/*    if (!(image = fopen("convertible_2.raw", "rb"))) {
        cout << "Cannot open file: " << "convertible_2.raw" << endl;
        exit(1);
    }
...
//  num_temp=extract_siftfeatures(image_temp,keypoint_temp,descriptor_temp);
  ...
  imshow("src",img_keypoint1);
  //imwrite("SUV_SURF.jpg",img_keypoint1);
  imshow("temp",img_keypoint2);
  //imwrite("Truck_SURF.jpg",img_keypoint2);
  waitKey(30);
  ////////////////////////////////
  vector<DMatch> good_matches;

  int num_good=match_knnfeatures(descriptor_src, descriptor_temp, good_matches);
  cout<<"good_num="<<num_good<<endl;

  Mat img_matches;
  drawMatches(image_src,keypoint_src,image_temp,keypoint_temp,good_matches,img_matches);
